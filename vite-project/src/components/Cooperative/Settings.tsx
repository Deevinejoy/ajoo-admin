...
const Settings: React.FC = () => {
  ...
  const handleEditRole = async (roleId: number) => {
    setRoleSaving(true);
    try {
      const response = await fetch(`https://ajo.nickyai.online/api/v1/cooperative/roles/${roleId}`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`,
          'Content-Type': 'application/json',
      ...
      const rolesResponse = await fetch(`https://ajo.nickyai.online/api/v1/cooperative/roles/${cooperativeId}`, {
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`,
          'Content-Type': 'application/json',
          'Accept': 'application/json',
  ...
  const handleAddRole = async (e: React.FormEvent) => {
    ...
    setAddingRole(true);
    try {
      const response = await fetch('https://ajo.nickyai.online/api/v1/cooperative/roles', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`,
          'Content-Type': 'application/json',
