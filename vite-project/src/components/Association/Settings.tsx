...
const Settings: React.FC = () => {
  ...
  return (
    <div className="p-3 md:p-6">
      ...
      <p className="text-sm md:text-base text-[#939393] mb-4 md:mb-6">Manage cooperative preferences and configurations</p>
      {/* Tabs */}
      <div className="flex flex-wrap gap-2 bg-[#ECEFF1] rounded-lg p-2 mb-6 md:mb-8 overflow-x-auto">
        {TABS.map((t) => (
          <button
            key={t.label}
            className={`flex items-center gap-1 md:gap-2 px-2 md:px-4 py-2 rounded-md font-medium text-xs md:text-base whitespace-nowrap ${tab === t.label ? 'bg-white shadow text-[#111827]' : 'text-[#939393]'}`}
      ...
      {tab === 'Permission' && (
        <div className="bg-white rounded-lg p-4 md:p-8">
          ...
          <p className="text-sm md:text-base text-[#939393] mb-4 md:mb-6">Configure access levels for cooperative administrators and users</p>
          {/* Admin Card */}
          <div className="bg-white border border-[#E5E5E5] rounded-lg p-4 md:p-6 mb-4 md:mb-6">
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-4 sm:mb-2 gap-2">
              <div>
              ...
              <button className="bg-[#F5F7FA] border border-[#C4C4C4] px-4 md:px-6 py-2 rounded-lg w-full sm:w-auto">Edit</button>
            </div>
          ...
          <button className="bg-[#111827] text-white px-4 md:px-6 py-2 rounded-lg w-full md:w-auto">Add Role</button>
        </div>
      )}
      {tab === 'Loans' && (
        <div className="bg-white rounded-lg p-4 md:p-8">
          <h2 className="text-lg md:text-xl font-semibold mb-1">Loan Settings</h2>
          <p className="text-sm md:text-base text-[#939393] mb-4 md:mb-6">Configure loan parameters and repayment terms</p>
          {editingLoanIdx === null ? (
            <div className="bg-[#FAFAFA] rounded-xl p-4 md:p-6">
              <div className="overflow-x-auto">
                <table className="w-full mb-6 min-w-[800px]">
                  <thead>
              ...
              <button className="bg-[#111827] text-white px-4 md:px-6 py-2 rounded-lg w-full md:w-auto">Add Loan</button>
            </div>
          ) : (
            <form className="bg-[#FAFAFA] rounded-xl p-4 md:p-8 grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
              ...
              <p className="col-span-1 md:col-span-2 text-sm md:text-base text-[#939393] mb-4 md:mb-6">Configure loan parameters and repayment terms</p>
              <div className="col-span-1 md:col-span-2">
              ...
              <div className="col-span-1 md:col-span-2 mt-4">
                <button type="button" className="bg-[#111827] text-white px-4 md:px-6 py-2 rounded-lg w-full md:w-auto" onClick={() => setEditingLoanIdx(null)}>Save Changes</button>
              </div>
            </form>
          )}
        </div>
      ...
      {tab === 'Profile' && (
        <div className="bg-white rounded-lg p-4 md:p-8">
          ...
          <p className="text-sm md:text-base text-[#939393] mb-4 md:mb-6">Manage your personal information</p>
          <div className="flex flex-col sm:flex-row items-center gap-4 md:gap-6 mb-6 md:mb-8">
            <div className="w-16 h-16 md:w-24 md:h-24 rounded-full bg-[#D6E6EF] flex items-center justify-center text-xl md:text-3xl font-medium text-[#22223B]">
              JD
            </div>
            <button className="bg-[#F5F7FA] border border-[#C4C4C4] px-4 md:px-6 py-2 rounded-lg w-full sm:w-auto">Upload Photo</button>
          </div>
          <form className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
            <div>
